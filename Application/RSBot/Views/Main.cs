...
public partial class Main : UIWindow
{
    ...
    {
        tabListBox.DrawItem += TabListBox_DrawItem;
    ...
    private void TabListBox_DrawItem(object sender, DrawItemEventArgs e)
    {
        ...
        {
            backColor = isDark ? Color.FromArgb(35, 35, 38) : Color.FromArgb(245, 245, 245);
            textColor = isDark ? Color.FromArgb(200, 200, 200) : Color.FromArgb(60, 60, 60);
        }
    ...
    private bool _isChangingTab = false;  // ⭐ أضف هذا المتغير في أول الـ class

    private void TabListBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (_isChangingTab) return;
        if (tabListBox.SelectedIndex < 0) return;
        if (windowPageControl == null) return;
        if (windowPageControl.Controls.Count == 0) return;
        if (tabListBox.SelectedIndex >= windowPageControl.Controls.Count) return;

        try
        {
            _isChangingTab = true;
            windowPageControl.SelectedIndex = tabListBox.SelectedIndex;
        }
        catch
        {
            // تجاهل الأخطاء
    ...
    private void WindowPageControl_SelectedIndexChanged(object sender, int e)
    {
        ...
        if (tabListBox.Items.Count == 0) return;
        if (e >= tabListBox.Items.Count) return;

        try
        {
            _isChangingTab = true;
            tabListBox.SelectedIndex = e;
        }
        catch
        {
            // تجاهل الأخطاء
    ...
    private void RefreshSidebarTabs()
    {
        ...
        try
        {
            ...
            foreach (Control control in windowPageControl.Controls)
            {
                ...
                {
                    tabListBox.Items.Add(control.Text);
                }
            }
        }
        catch
    ...
    public void RefreshTheme(bool save = true)
    {
        ...
        // تحديث ألوان الـ Sidebar مع الثيم
        bool isDark = BackColor.IsDark();
        sidebarPanel.BackColor = isDark ? Color.FromArgb(35, 35, 38) : Color.FromArgb(245, 245, 245);
        tabListBox.BackColor = sidebarPanel.BackColor;
        tabListBox.ForeColor = isDark ? Color.White : Color.Black;
        tabListBox.Invalidate();
    ...
    private async Task SelectBotbase(string name)
    {
        ...
        // ⭐ فقط غيّر الـ Botbase النشط (بدون إضافة للـ Controls)
        ...
        // ⭐ انتقل للتاب المحدد
        var index = windowPageControl.Controls.IndexOfKey(name);
        if (index >= 0)
            windowPageControl.SelectedIndex = index;
    }
    ...
    private void LoadExtensions()
    {
        ...
        // تحديث الـ Sidebar بعد إضافة كل الـ Extensions
        RefreshSidebarTabs();
    ...
    {
        sidebarPanel.Visible = menuSidebar.Checked;
    }
    ...
    private void LanguageDropdown_Click(object sender, EventArgs e)
    {
        ...
        // تحديث الـ Sidebar بعد تغيير اللغة
        RefreshSidebarTabs();
    ...
    private void OnLoadBotbases()
    {
        ...
        foreach (var bot in Kernel.BotbaseManager.Bots)
        {
            ...
            // ⭐ أضف الـ View للـ Tabs
            bot.Value.Translate();
            var control = bot.Value.View;
            control.Name = bot.Value.Name;
            control.Text = bot.Value.TabText;
            control.Dock = DockStyle.Fill;
        ...
        // اختر الـ Botbase الافتراضي
        var defaultBot = GlobalConfig.Get("RSBot.BotName", "RSBot.Training");
        var selectedBot = Kernel.BotbaseManager.Bots.FirstOrDefault(b => b.Value.Name == defaultBot);
        if (selectedBot.Value != null)
        {
            Kernel.Bot?.SetBotbase(selectedBot.Value);
        }
    }
