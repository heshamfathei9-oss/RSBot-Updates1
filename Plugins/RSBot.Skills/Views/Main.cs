...
public partial class Main : DoubleBufferedControl
{
    ...
    {
        if (listAttackingSkills.SelectedItems.Count == 0)
            return;

        var items = listAttackingSkills.Items.Cast<ListViewItem>().ToList();
        var selectedIndexes = listAttackingSkills.SelectedItems.Cast<ListViewItem>()
            .Select(item => listAttackingSkills.Items.IndexOf(item))
            .OrderByDescending(index => index)
            .ToList();

        // تحريك العناصر المحددة لأسفل
    ...
    {
        // إذا مفيش عناصر محددة نخرج فوراً
        if (listBuffs.SelectedItems.Count == 0)
            return;

        // ننسخ العناصر المحددة لمصفوفة لتجنب تعديل المجموعة أثناء التكرار
        var items = listBuffs.SelectedItems.Cast<ListViewItem>().ToArray();
    ...
    private void button2_Click_1(object sender, EventArgs e)
    {
        ...
        var items = listBuffs.Items.Cast<ListViewItem>().ToList();
        var selectedIndexes = listBuffs.SelectedItems.Cast<ListViewItem>()
            .Select(item => listBuffs.Items.IndexOf(item))
            .OrderBy(index => index)
            .ToList();
    ...
    private void panelPlayerSkills_Paint(object sender, PaintEventArgs e)
    {
