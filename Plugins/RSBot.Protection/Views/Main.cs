...
public partial class Main : DoubleBufferedControl
{
    ...
    private void LoadSettings()
    {
        ...
        foreach (var num in groupStatPoints.Controls.OfType<NumUpDown>())
            num.Value = PlayerConfig.Get(key + num.Name, num.Value);
        checkBox2.Checked = PlayerConfig.Get<bool>(key + "checkBackTownIfDeadOutside", false);
    ...
    private void checkBox2_CheckedChanged(object sender, EventArgs e)
    {
        if (_settingsLoaded)
        {
            PlayerConfig.Set("RSBot.Protection.checkBackTownIfDeadOutside", checkBox2.Checked);
        }
    }
}
