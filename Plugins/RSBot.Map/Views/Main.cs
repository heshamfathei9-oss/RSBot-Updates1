...
public partial class Main : DoubleBufferedControl
{
    /// <summary>
    ///     The grid size. 
    ///     تم التعديل: 5 بدلاً من 3 لمنع ظهور المناطق السوداء
    /// </summary>
    private const int GridSize = 7;

    /// <summary>
    ///     حجم شبكة التحميل المسبق (يجب أن تكون أكبر أو تساوي حجم الشبكة)
    /// </summary>
    private const int PreloadGridSize = 7;
    ...
    /// <summary>
    ///     للتحقق من أن التحميل المسبق يعمل
    /// </summary>
    private bool _isPreloading = false;

    /// <summary>
    ///     آخر موقع تم التحميل المسبق له
    /// </summary>
    private byte _lastPreloadX;
    private byte _lastPreloadY;

    /// <summary>
    ///     معلومات الدنجن الحالية للتخزين
    /// </summary>
    ...
    private void OnEnterGame()
    {
        ...
        Task.Run(() => PreloadSurroundingSectors());
    }
    ...
    private Image LoadSectorImage(string sectorImgName)
    {
        ...
        var transparentBitmap = new Bitmap(SectorSize, SectorSize, PixelFormat.Format32bppArgb);
    ...
    private void PreloadSectorImage(string sectorImgName)
    {
        lock (_cachedImages)
        {
            if (_cachedImages.ContainsKey(sectorImgName))
    ...
    private void PreloadSurroundingSectors()
    {
        if (_isPreloading || Game.Player == null)
            return;
        ...
        try
        {
            ...
            byte centerX = p.Region.X;
            byte centerY = p.Region.Y;

            if (p.Region.IsDungeon)
            {
                centerX = p.GetSectorFromOffset(p.XOffset);
                centerY = p.GetSectorFromOffset(p.YOffset);
            }

            if (Math.Abs(centerX - _lastPreloadX) < 1 && Math.Abs(centerY - _lastPreloadY) < 1)
            {
                _isPreloading = false;
                return;
            }

            _lastPreloadX = centerX;
            _lastPreloadY = centerY;

            UpdateDungeonInfo(p);

            int halfGrid = PreloadGridSize / 2;
            for (int x = -halfGrid; x <= halfGrid; x++)
            {
                for (int y = -halfGrid; y <= halfGrid; y++)
                {
                    var xSector = (byte)(centerX + x);
                    var ySector = (byte)(centerY + y);

                    var sectorImgName = GetMinimapFileName(new Region(xSector, ySector), _currentDungeonName, _currentFloorName);
                    PreloadSectorImage(sectorImgName);
                }
            }
        }
        catch (Exception ex)
        {
            Log.Debug($"[Map] Preload error: {ex.Message}");
        }
        finally
        {
            _isPreloading = false;
    ...
    private void UpdateDungeonInfo(Position p)
    {
        if (p.Region.IsDungeon)
        {
            ...
            _currentFloorName = dungeon.FloorStringIDs[dungeonBlock.FloorIndex];
            _currentDungeonName = RegionInfoManager.GetDungeonName(p.Region);
        }
        else
        {
            _currentFloorName = string.Empty;
    ...
    private void RedrawMap()
    {
        ...
        if (Math.Abs(tempX - _lastPreloadX) >= 1 || Math.Abs(tempY - _lastPreloadY) >= 1)
        {
            Task.Run(() => PreloadSurroundingSectors());
        }

        if (tempX == _currentXSec && tempY == _currentYSec && _currentSectorGraphic != null)
            return;

        _currentXSec = tempX;
        _currentYSec = tempY;

        // زيادة سعة الكاش لتحمل الشبكة الأكبر
        ...
        try
        {
            ...
            // تكبير الصورة لتستوعب 5x5 قطاعات
            ...
            {
                if (p.TryGetNavMeshTransform(out var pTransform) && 
                    pTransform.Instance is NavMeshInstBlock dungeonBlock && 
                    dungeonBlock.Parent is NavMeshDungeon dungeon)
                {
                    ...
                    _regionName = Game.ReferenceManager.GetTranslation(roomName);
            ...
            _currentDungeonName = dungeonName;
            _currentFloorName = floorName;

            // حساب الإزاحة (لشبكة 5 تكون الإزاحة 2)
            int offset = (GridSize - 1) / 2;

            for (int x = 0; x < GridSize; x++)
            {
                for (int z = 0; z < GridSize; z++)
                {
                    // تعديل حساب القطاعات بناءً على الإزاحة الجديدة
                    var xSector = (byte)(_currentXSec + x - offset);
                    var ySector = (byte)(_currentYSec + z - offset);
    ...
    private void DrawObjects(Graphics graphics)
    {
        if (_currentSectorGraphic != null)
        {
            ...
            // حساب نقطة المنتصف بناءً على حجم الشبكة الجديد
            float centerOffset = (GridSize - 1) / 2f; // يساوي 2 لشبكة 5

            PointF point = new()
            {
                // تعديل المعادلة لضبط الصورة الكبيرة في المنتصف
                X = mapCanvas.Width / 2f - (SectorSize * centerOffset) - Game.Player.Movement.Source.XSectorOffset / 10f * _scale,
                Y = mapCanvas.Height / 2f - (SectorSize * (centerOffset + 1)) + Game.Player.Movement.Source.YSectorOffset / 10f * _scale,
            };
