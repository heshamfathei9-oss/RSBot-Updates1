...
internal partial class Main : DoubleBufferedControl
{
    ...
    // متغير لحفظ الحجم الأصلي للنافذة
    private Size _originalFormSize = Size.Empty;
    ...
    private void LoadAccounts()
    {
        ...
        var autoLoginUserName = GlobalConfig.Get<string>("RSBot.General.AutoLoginAccountUsername") ?? string.Empty;
        autoLoginUserName = autoLoginUserName.ToLowerInvariant();
        ...
        foreach (var account in Accounts.SavedAccounts)
        {
            ...
            if (!string.IsNullOrEmpty(autoLoginUserName) &&
                ...
                account.Username.ToLowerInvariant() == autoLoginUserName)
            {
    ...
    private void domainUpDown1_SelectedItemChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            string scaleText = domainUpDown1.Text.Replace("%", "").Trim();

            if (!int.TryParse(scaleText, out int scalePercent))
                return;
            ...
            // حفظ الحجم الأصلي في أول مرة
            if (_originalFormSize.IsEmpty)
                _originalFormSize = form.Size;

            // حساب الحجم الجديد
            float factor = scalePercent / 100f;

            form.Size = new Size(
                (int)(_originalFormSize.Width * factor),
                (int)(_originalFormSize.Height * factor)
            );
        }
        catch (Exception ex)
        {
            Log.Error($"Error changing scale: {ex.Message}");
        }
    }
}
