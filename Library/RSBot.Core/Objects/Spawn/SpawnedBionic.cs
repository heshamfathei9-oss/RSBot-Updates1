...
public class SpawnedBionic : SpawnedEntity
{
    ...
    {
        // ✅ لو في تحديد يدوي نشط، متحددش تلقائياً
        if (Game.IsManualSelectActive)
        {
            Log.Debug($"[TrySelect] Skipping auto-select, manual select is active");
            return false;
        }

        // لو الهدف ده محدد أصلاً
    ...
    {
        // ✅ لو في تحديد يدوي نشط، متلغيش التحديد
        if (Game.IsManualSelectActive)
        {
            Log.Debug($"[TryDeselect] Skipping auto-deselect, manual select is active");
            return false;
        }
