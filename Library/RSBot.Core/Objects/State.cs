...
public class State
{
    ...
    public IEnumerable<object> Buffs { get; set; }
    ...
    public bool HasActiveBuff(SkillInfo skill, out SkillInfo buff)
    {
        ...
        string skillName = skill.Record.GetRealName();

        // ⭐ الطريقة 1: نفس الـ ID
        buff = ActiveBuffs.Find(p => p.Id == skill.Id);
        if (buff != null)
            return true;
        ...
        buff = ActiveBuffs.Find(p => 
            p.Record != null && 
            p.Record.GetRealName() == skillName
        );
        if (buff != null)
            return true;

        // ⭐ الطريقة 3: الـ Group يبدأ بنفس البداية (CASTER_B يلاقي CASTER_B_DD)
        buff = ActiveBuffs.Find(p =>
            p.Record != null &&
            (p.Record.Basic_Group.StartsWith(skill.Record.Basic_Group) ||
             skill.Record.Basic_Group.StartsWith(p.Record.Basic_Group))
        );
    ...
    public bool HasActiveBuff(uint id)
    {
